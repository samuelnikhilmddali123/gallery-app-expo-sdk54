...
const NUM_COLUMNS = 3;
const GAP = 3;
const getItemSize = (screenWidth) => (screenWidth - GAP * (NUM_COLUMNS + 1)) / NUM_COLUMNS;

// Memoized MediaItem component for performance - defined outside to avoid hooks issues
const MediaItem = React.memo(({ item, index, isSelected, onPress, onLongPress, colors, size }) => {
  return (
    <TouchableOpacity
      activeOpacity={0.9}
      style={[
    ...
    >
      ...
      {(item.mediaType === 'video' || item.mediaType === MediaLibrary.MediaType.video || item.duration > 0) && (
...
export default function AlbumViewScreen({ route, navigation }) {
  ...
  const loadAlbumMedia = useCallback(async () => {
    try {
      ...
      const currentAlbum = albums.find(a => a.id === album.id) || album;
      setAlbumInfo(currentAlbum);

      // Get all photos from the album
      const photosRes = await MediaLibrary.getAssetsAsync({
      ...
      const videosRes = await MediaLibrary.getAssetsAsync({
        album: currentAlbum.id,
  ...
  const handleShareSelected = useCallback(async () => {
    ...
    try {
      ...
      console.log(`AlbumView: Sharing ${uris.length} items via NativeModules.MultiShare`);
      ...
      if (!errorMsg.includes('User did not share') && !errorMsg.includes('User cancelled')) {
        console.error('AlbumView: Share error:', errorMsg);
  ...
  }, [selectedItems, media, exitSelectionMode, showAlert, showCustomConfirm]);
  ...
  }, [selectedItems, handleItemPress, handleLongPress, colors, dimensions.width]);

  const keyExtractor = useCallback((item, index) => {
    return item?.id?.toString() || `album-media-${index}`;
  }, []);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerInfo}>
            <Text style={[styles.title, { color: colors.text }]} numberOfLines={1}>
              {String(albumInfo?.title || album?.title || 'Album')}
            </Text>
      ...
      {media.length === 0 ? (
        ...
        <FlatList
          key={`album-${dimensions.width}x${dimensions.height}`}
          data={media}
